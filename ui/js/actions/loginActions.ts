...
// Action creators
export function loginAct(email: string, pwd: string) { //firstName: string, lastName: string, 
  return (dispatch: any, getState: any) => {
    dispatch({ type: TOGGLE_LOADER, status: true, loaderTxt: 'Signing in...' });

    fetch('http://localhost:3000/users/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' }, // Must have, otherwise fetch doesn't work
      body: JSON.stringify({
        // firstName: firstName,
        // lastName: lastName,
        email: email,
